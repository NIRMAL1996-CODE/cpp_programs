//write a program to find the maximum element in a linkedlist//
#include<iostream>
#include<stdlib.h>
using namespace std;
/* create a node:-
...
struct node *first=NULL;//pointer to create a 1st node(can declare as global or local & initialise it with NULL as right now its empty)//
...
{   int i;
    ...
    first=(struct node*)malloc(sizeof(struct node));//create a memory 1st using malloc function and tycaste with struct node datatype as we need that kind of memory//
    first->data=a[0];//with pointers we always use arrow to point any variable//
    first->next=NULL;
    ...
    lastnode=first;//as no next node is their,first will become last//
